...
impl McpServer {
    ...
    fn list_resources(&self) -> Value {
        ...
        let session_list: Vec<String> =
            runtime.sessions.keys().map(|k| k.clone()).collect();
            ...
            "resources": [
                {
                    ...
                    "description": format!("{session_count} tracked sessions"),
        ...
        match name {
            ...
            "get_routing_suggestion" => {
                ...
                let result = match router.route(&ctx) {
                    Some(nudge) => json!({"prompt": prompt, "suggestion": nudge}),
                    None => json!({"prompt": prompt, "suggestion": null}),
                };
                json!({
                    "content": [{"type": "text", "text": serde_json::to_string_pretty(&result).unwrap_or_default()}],
                })
...
pub fn run() {
    ...
    for line in reader.lines().map_while(Result::ok) {
        ...
        // Handle notifications (no id) — just ack with nothing
