...
pub fn classify_pure(
    ...
    match subcommand {
        ...
        "checkout" | "switch" => {
            ...
            if is_protected_branch(target, Some(default_branch)) && !working_tree_clean {
                Disposition::Deny {
                    reason: format!(
...
/// I/O-resolving entry point: resolves the default branch, (for `push` with
/// a resolvable branch/target pair) whether the push touches a trust-root
/// path, and (for a checkout/switch onto the default branch) whether the
/// working tree is clean, then delegates to `classify_pure`.
#[must_use]
...
pub fn classify_with_home(
    ...
    // Only pay for a `git status --porcelain` when it can actually change the
