...
/// `[item_id, agent]`, exactly what `WorkItemExecutor::execute` expects.
///
/// Shared by `dispatch_item` (a fresh `ready-for-work` item) and
/// `run_review_sweep`'s self-repair path (item #65, re-running the same job
/// on an item already sitting in "in_review" -- `item_claim` reclaims its
/// existing worktree/branch rather than starting over, see `item::claim`'s
/// doc comment).
fn enqueue_work_job(
    queue: &agentflare_jobs::Queue,
    item: &agentflare_backend::item::Item,
...
fn dispatch_item(
    ...
    let Some(info) = enqueue_work_job(queue, item, agent) else {
        return false;
    };
