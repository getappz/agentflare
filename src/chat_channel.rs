...
///
/// `pub(crate)` so `supervisor::notify`'s side-channel card sends (PR/plan
/// approval gates) can take the same lock before writing to a chat this
/// module might simultaneously be replying in -- otherwise the two Telegram
/// API calls race with no ordering between them, and an unrelated card can
/// land interleaved with an in-flight chat reply (item #281).
pub(crate) fn chat_turn_lock(chat_id: &str) -> std::sync::Arc<std::sync::Mutex<()>> {
...
/// Prepend a note to a chat reply that was generated by a brand-new
/// (zero-history) session after `run_chat_turn` detected the resumed one had
/// gone stale. Pulled out of `run_chat_turn` as pure text logic so it's
/// directly unit-testable without a real subprocess.
fn prefix_session_reset_note(reply: &str) -> String {
    format!(
        "[my memory of this chat was reset -- replying fresh, with no earlier context]\n\n{reply}"
    )
}
...
fn run_chat_turn(channel: &dyn ChatChannel, chat_id: String, prompt: String) {
    ...
    // Set once the resumed session turns out to be gone (e.g. evicted, or
    // the daemon restarted since -- item #159's failure mode) and this turn
    // falls back to a brand-new one with zero history. A fresh session
    // asked something like "Approved" with no idea what it refers to reads
    // to the human as the bot ignoring their pending request; it's actually
    // genuinely forgotten it (item #281's reported symptom), so this is
    // surfaced below instead of left silent.
    ...
    if session_was_reset {
        reply = prefix_session_reset_note(&reply);
    }
...
mod tests {
    ...
    #[test]
    fn prefix_session_reset_note_prepends_context_loss_note() {
        let noted = prefix_session_reset_note("I don't have any prior context to approve.");
        assert!(noted.starts_with("[my memory of this chat was reset"));
        assert!(noted.ends_with("I don't have any prior context to approve."));
    }
