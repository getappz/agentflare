// Regression coverage for #550's worktree_error handling in
// `execute_work_impl`. Split out of `work.rs` to keep that file under the
// LOC gate. Included from `work::tests` via `include!`.
...
/// (`mcp_server::tests::action_tests`) covers. Guards that the
/// `claim["worktree_error"]` detail (fix #550) is actually wired into this
/// call site's `release_and_comment`, not just parsed and dropped.
#[test]
