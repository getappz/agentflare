...
impl AgentflareMcp {
    ...
    fn item_inner(&self, req: ItemRequest) -> Result<String, ErrorData> {
        match req.action.as_str() {
            ...
            other => Err(ErrorData::invalid_params(
                format!(
                    "unknown item action: '{other}' — expected create|get|list|search|update|update_state|delete|claim|heartbeat|release|done|check_merge|cancel|add_label|remove_label|redispatch|groom|standup|health|doctor|clear_start_date|clear_due_date"
                ),
                None,
            )),
        }
