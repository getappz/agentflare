...
impl AgentflareMcp {
    ...
    async fn skill_detect(&self, Parameters(req): Parameters<SkillDetectRequest>) -> Result<String, ErrorData> {
        if req.prompt.trim().is_empty() {
            return Err(ErrorData::invalid_params("prompt is required", None));
        }
        let intent = crate::skill_detect::classify(&req.prompt);
        let limit = req.limit.unwrap_or(3).max(1).min(10);
        let skills = self.with_fresh_registry(|reg| {
            crate::skill_detect::find_skills(&intent, reg, limit)
        ...
        let body_skills = if req.include_body {
            let loaded: Vec<serde_json::Value> = self.with_fresh_registry(|reg| {
                skills.iter().map(|s| {
                    match reg.load(&s.name, false) {
