...
impl AgentflareMcp {
    ...
    fn item_inner(&self, req: ItemRequest) -> Result<String, ErrorData> {
        match req.action.as_str() {
            ...
            "review_result" => self.item_review_result(req),
            ...
            "approve_plan" => self.item_approve_plan(req),
            "reject_plan" => self.item_reject_plan(req),
            other => Err(ErrorData::invalid_params(
                format!(
                    "unknown item action: '{other}' — expected create|get|list|search|update|update_state|delete|claim|heartbeat|release|done|check_merge|cancel|pause|resume|add_label|remove_label|add_relation|remove_relation|list_relations|redispatch|review_result|groom|standup|health|doctor|status|clear_start_date|clear_due_date|submit_plan|approve_plan|reject_plan"
                ),
                None,
            )),
        }
