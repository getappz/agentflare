...
/// `unwrap_or`+`clamp` pattern above; the daemon log itself is unbounded
/// (only truncated on daemon restart, see `daemon::daemon_log_path`'s doc
/// comment), so an unbounded return here could dump an entire session's log.
...
fn recent_log_lines_at(
    ...
    let matching: Vec<&str> = contents
        .lines()
        .filter(|line| line.contains(&seq_marker) || line.contains(item.id.as_str()))
        .collect();
...
impl AgentflareMcp {
    ...
    /// One-call status aggregator (item #298): item state + its most recent
    /// dispatch job + PR CI status + recent daemon-log lines mentioning it,
    /// so checking progress on a dispatched item doesn't need `get` +
    /// `workflow(action="status")` + `check_merge` + a manual
